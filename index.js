...
const demo = async () => {
  ...
  // Make a prediction through the locally hosted loadpageimage.jpg.
  const loadPageElement = document.getElementById('loadpageimage');
  if (loadPageElement.complete && loadPageElement.naturalHeight !== 0) {
    predict(loadPageElement);
    loadPageElement.style.display = '';
  } else {
    loadPageElement.onload = () => {
      predict(loadPageElement);
      loadPageElement.style.display = '';
    }
  }
